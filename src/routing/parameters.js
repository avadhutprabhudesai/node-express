...
app.get('/users/:id', (req, res) => {
  logger(`${__dirname.split('src')[1]}`, 'GET /users/:id on', PORT);
  res.write('Single positional parameter\n');
  res.end(JSON.stringify(req.params));
});

app.get('/users/:id/post/:postId', (req, res) => {
  logger(`${__dirname.split('src')[1]}`, 'GET /users/:id/post/:postId', PORT);
...
app.get('/users/:id/journey/:src-:dest', (req, res) => {
  logger(
    `${__dirname.split('src')[1]}`,
    'GET /users/:id/journey/:src-:dest on',
    PORT
  );
...
app.get('/user/:id([0-9]+)', (req, res) => {
  logger(`${__dirname.split('src')[1]}`, 'GET /users/:id(/d+/) on', PORT);
  res.write(
